...
class Net(torch.nn.Module):
    # upscale_factor=1 means no changed
    def __init__(self, num_channels, base_filter, upscale_factor):
        super(Net, self).__init__()

        # define the each layer
        self.layers = torch.nn.Sequential(
            nn.Conv2d(in_channels=num_channels, out_channels=base_filter,
                      kernel_size=9, stride=1, padding=4, bias=True),

            nn.ReLU(inplace=True),

            # nn.Conv2d(in_channels=base_filter, out_channels=base_filter // 2,
            #           kernel_size=1, stride=1, padding=0, bias=True),
            #
            # nn.ReLU(inplace=True),

            nn.Conv2d(in_channels=base_filter, out_channels=upscale_factor ** 2,
    ...
    # initial the weights use mean=0, std=1
...
class SRCNNTrainer(object):
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_num, (data, target) in enumerate(self.testing_loader):
                ...
                prediction_total.append(prediction_array[0, :, :, :])
        ...
        return contentPSNR, contentSSIM, prediction_total
    ...
    def run(self):
        ...
        for epoch in range(1, self.nEpochs + 1):
            ...
            self.train()
            contentPSNR, contentSSIM, prediction_total = self.test()
            ...
            if epoch == self.nEpochs:
                # print("\n prediction: \n" + str(prediction_total))
                # print("\n prediction: \n" + str(prediction_total[4]))
                # print("\n shape of prediction_total: " + str(np.shape(prediction_total)))
                for i in range(5):
                    PILImageSet.append(prediction_total[i])
                    # PILImageSet = ToPILImage(prediction_total[i])


        return contentPSNRTotal, contentSSIMTotal, PILImageSet
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.input_transform:
            # use gaussian_filter process each image, sigma=2
            # img = self.input_transform(input_image)
            # input_image = filters.gaussian_filter(img, 2)
...
def progress_bar(current, total, msg=None):
    ...
    time_used += ' | Total: %s' % format_time(total_time)
    if msg:
        time_used += ' | ' + msg
...
def main():
    ...
    # SRCNNTrainer(args, training_data_loader).run()
    resultPSNR, resultSSIM, PILImageSet = SRCNNTrainer(args, training_data_loader, testing_data_loader).run()


    for i in range(5):
        plt.imshow(PILImageSet[i][0], cmap='gray')
        plt.show()
    # print("\n PILImageSet: \n" + str(PILImageSet))
    # print("\n PILImageSet: \n" + str(np.shape(PILImageSet)))
    # print("\n PILImageSet: \n" + str(PILImageSet[4]))
    # print("\n PILImageSet: \n" + str(np.shape(PILImageSet[4])))  # shape: (1, 480, 360)
